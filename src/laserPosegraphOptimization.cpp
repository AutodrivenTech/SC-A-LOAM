...
void writeEdge(const std::pair<int, int> _node_idx_pair,
...
void saveGTSAMgraphG2oFormat(const gtsam::Values &_estimates)
{
  ...
  for (auto &_line : edges_str) pgG2oSaveStream << _line << std::endl;
...
void initNoises(void)
{
  ...
  robustLoopNoise = gtsam::noiseModel::Robust::Create(
    gtsam::noiseModel::mEstimator::Cauchy::Create(1),// optional: replacing Cauchy by DCS or GemanMcClure is okay but
...
Pose6D diffTransformation(const Pose6D &_p1, const Pose6D &_p2)
{
  ...
  return Pose6D{
    double(abs(dx)), double(abs(dy)), double(abs(dz)), double(abs(droll)), double(abs(dpitch)), double(abs(dyaw))
  };
}// SE3Diff
...
void pubPath(void)
{
  ...
  for (int node_idx = 0; node_idx < recentIdxUpdated; node_idx++)// -1 is just delayed visualization (because sometimes
...
pcl::PointCloud<PointType>::Ptr transformPointCloud(pcl::PointCloud<PointType>::Ptr cloudIn, gtsam::Pose3 transformIn)
{
  ...
  Eigen::Affine3f transCur = pcl::getTransformation(transformIn.translation().x(),
    transformIn.translation().y(),
    transformIn.translation().z(),
    transformIn.rotation().roll(),
    transformIn.rotation().pitch(),
    transformIn.rotation().yaw());
  ...
  const int &root_idx)
{
  ...
  for (int i = -submap_size; i <= submap_size; ++i) {
    int keyNear = key + i;// see https://github.com/gisbi-kim/SC-A-LOAM/issues/7 ack.
                          // @QiMingZhenFan found the error and modified as below.
...
{
  // parse pointclouds
  int historyKeyframeSearchNum = 25;// enough. ex. [-25, 25] covers submap length of 50x1 = 50m if every
                                    // kf gap is 1m
...
void process_pg()
{
  while (1) {
    while (!odometryBuf.empty() && !fullResBuf.empty()) {
      ...
      mBuf.lock();
      while (
        !odometryBuf.empty() && odometryBuf.front()->header.stamp.toSec() < fullResBuf.front()->header.stamp.toSec())
        odometryBuf.pop();
      ...
      const int curr_node_idx = keyframePoses.size() - 1;// becuase cpp starts with 0 (actually this index could be any
      ...
      {// == keyframePoses.size() > 1
        ...
        mtxPosegraph.lock();
        {
          ...
          if (hasGPSforThisKF) {
            ...
            mtxRecentPose.lock();
            gtsam::Point3 gpsConstraint(recentOptimizedX,
              recentOptimizedY,
          ...
          writeEdge({ prev_node_idx, curr_node_idx },
            relPose,
            edges_str);// giseop
                       // runISAM2opt();
        }
        ...
        if (curr_node_idx % 100 == 0) cout << "posegraph odom node " << curr_node_idx << " added." << endl;
      }
...
int main(int argc, char **argv)
{
  ...
  // pgG2oSaveStream = std::fstream(save_directory +
  // "singlesession_posegraph.g2o", std::fstream::out);
  ...
  std::thread isam_update{ process_isam };// if you want to call less isam2 run (for saving redundant
